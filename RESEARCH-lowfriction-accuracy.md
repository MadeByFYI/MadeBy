...
> untrailered-recall headline this study interrogates), `STRATEGY.md §4` (cold-start / the flywheel
> ignition point), and `TESTING.md §2` (the fail-safe / severity-inversion doctrine under test).
> **Owner:** trust-QA / classification. **Decision it informs:** whether to build a probabilistic
> detection tier on the low-friction path, and if so, under what constraints.

---

## 0. Executive summary

The `customer_development.md` evaluation named one root objection: **the accurate path
(`capture --local`) is high-friction, the low-friction path (the web "curiosity mirror") is
inaccurate, and that gap sits exactly on the flywheel's ignition point.** The low-friction detector
is trailer-only and its measured recall on untrailered AI is **0%** — worst for the heaviest AI
users, who are the exact vibe-coder audience the GTM targets.
...
3. **The real answer to the objection is not a detector at all** — it is coverage-led framing plus a
   ...
   accuracy gap today, so the flywheel bet belongs on making the *accurate* path trivial.
...
2. **Reframe detection as routing, not attribution — this is the unlock.** The blind-spot user needs
   ...
   - **on-brand** — coverage-led framing already shipped (#79/#87; `STRATEGY.md:476–483`);
   - **the upgrade CTA** the flywheel needs, placed at first contact;
...
4. **The highest-leverage lever is not a detector.** The study confirms the git-only accuracy gap
   cannot be closed honestly today. So the flywheel bet belongs where the strategy already points it:
   **coverage-led framing** (turn the blind spot into the CTA) and a **frictionless `capture --local`**
   (npx one-liner, second tool parser). Do not bet ignition on the detector; bet it on making the
   accurate path trivial.
...
- **Sharpened:** "frictionless capture" rises from a packaging nicety to the **conversion blocker**;
