...
|---|---|
| **Prompt injection via a malicious repo** (ingestion + the Claude-ops agent) | Containment designed for the ingestion + ops-agent boundary — untrusted repo content cannot reach secrets or escalate a claim's tier. Testable with ingestion (#10) / the ops agent. |
...
| **End-to-end badge image forgery** | A faked badge image carries no trust state; the resolver page is authoritative (badge is a pointer, never proof). Full integration test lands with the live badge service / resolver pages. |
