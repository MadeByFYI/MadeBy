...
> Companion to `TESTING.md` (how we keep it trustworthy). This document is the technical design v2
> is built against — *what* and *how*. The first attempt is in `archive/v1/`.
...
- **"Spine" ≠ "what ships first."** Architecturally everything keys on content from day one.
  Seeding still uses producer-push + public-git bootstrap (see §8), because an empty resolver
  returns "no record found." Build for pull; seed with push.
...
> human authorship — that nuance is tier/caveat, not a refusal to name. (The badge is a
> headline+pointer; the full answer lives on the resolver page.)
...
6. **Why** (when artifacts exist) — links to the motivating tickets / design docs / ADRs as **tiered
   edges** (declared vs. inferred), surfaced with their confidence (§6).
...
codegen-vs-generative detail **underneath**, never in the headline; **celebrate
...
This page is exactly where a trust product can slide into the Glassdoor **shame-to-claim** dark
pattern. Hard rule, a *standing refusal*: **celebrate disclosure, stay neutral on its absence —
"transparency you can *add*," never "shame you must *remove*."**
...
the **consent-gate for name-attached** negatives (§8), this keeps the page honest transparency
rather than reputation extortion.

#### Public page: assert the determination, point to the identity — don't host it (decision, 2026-08-11)

Detect-don't-host, extended to the *person* axis. The **public** resolver page shows
...
biggest PII risk-reducer: the public corpus becomes aggregate stats + human/machine
...
- **Privacy:** "anyone holding the bytes can query" is a leak vector for proprietary code.
  ...
  distribution to authorized askers) rides the live registry.
...
- **The registry is data, hosted and versioned — recognizers, not code (decision, 2026-08-12).** The
  ...
  is a durable advantage, **not lock-in**: the registry stays open for anyone to implement
  (`MISSION.md` value 6, "open, and no lock-in"), and the advantage is curation + default gravity,
  never trapping data. Two disciplines keep a hosted, growing
...
MadeBy owns. But it must not be welded to one ledger, exactly as the claim is not
...
ledger or stand up an identity.
...
tradeoff). Store = the registry; render = madeby.fyi.
...
and — because it is the default — concentrates data at the hosted resolution/grant layer, all
without our holding identity, ledger, or source.
...
reliance products need. (Distinct from the org-scoped *reporting-push* path — that is how records
reach an authority, not how files travel between consumers.) A consumer thus reads a
...
in-tree local read). Two properties make the traveled file trustworthy without a
...
traffic to the resolution surface. Default-profile requirement: ensure `.madeby` is **published with the
...
registry holds what is pushed, same honesty caution as travel). This is the deferred registry store
and the default-gravity resolution surface, now concretely motivated.
...
link to the resolution surface, riding a file that travels with each artifact to that artifact's consumers.
...
- **Signature-attachable by design — legal first, cryptographic optional.** The template carries a
  **legal signature block**: the declarant signs it (typed name + declaration + date, e-signature-
  ...
  consistent with `tiers.ts` ("no crypto binding, but legally consequential") and the orthogonal
  legal axis: bind the claim to *a person who bears consequences*, no cryptography needed.
...
- Modified/unrecognized text, or no identifiable signatory → capped at **asserted**, flagged
  ...
  sworn bar in a given jurisdiction is a legal question.)
...
vN"), we don't reproduce their claim — an intermediary posture, not a publisher's.
...
the **pointer** (path, template id, signatory, tier) — folded into the mirror. **The gate
...
The maintainer gate needs **no GitHub App and no hosted backend**. The carrier is a file:
**`.madeby/policy.json`**, in which a maintainer declares their disclosure policy:
...
(front-door `madewith.fyi`). `packages/action/` is the thin
composite Action that runs `npx madeby check` on a PR — so a maintainer adopts the whole flow by
committing two files (`.madeby/policy.json` + a workflow). As of 2026-08-13 the `madeby` package (and
the `@madeby` scope, via `@madeby/cli`) are published to npm — `0.1.0`, MIT — so the Action's
`npx madeby` reference now resolves.
...
a funding protocol needs. We do not process payments.
...
A verifiable portfolio also stands alone: reason to pull your work into MadeBy regardless of whether
...
- `IMPLEMENTS` / `MOTIVATED_BY` / `DECIDED_BY` / `DISCUSSED_IN` — link code to the **management
  ...
  non-code frontier. These edges are usually inferred, hence tiered.
...
- **Org / manager views** (the org index, §8): collaboration topology, **bus-factor / key-person
...
> fair-credit company). The contributor controls disclosure of their own graph. An ethical
...
fabricates disclosure for the past.
...
> **Metadata-only** (identity/trailers/signatures/paths — *no content*), bounded/sampled; it powers
> cross-repo reach reads, the human/machine index, and demand-generated discovery pages. Ingest for
> aggregates + cross-reference; keep per-subject pages demand-generated and neutral (§1). The
> content-fingerprint corpus stays deferred.
...
- Substrate for the **"analyze your existing git history"** entry point and the public **resolver
  pages**.
...
> **Consent gate (design rule).** Publishing a public, indexable, **name-attached** "who made this —
> N% AI" page about a developer who never opted in — from a heuristic that's frequently wrong — is
> uniquely corrosive for a *trust* brand and carries privacy exposure (inferred personal data). So:
> **ingest broadly, but gate the public, name-attached AI-inference page behind a claim** (or
> owner-connected allowlist). **Pre-claim, show only the neutral "who"** (git authors), never an
> AI-% headline. The public index ships **aggregate-only — never a per-individual "X is an AI coder"
> page.** The salted/HMAC **private-fingerprint scheme** (specced in §2) keeps private content
> non-enumerable, and a self-serve **opt-out + takedown** endpoint + `privacy@` is part of the
> public path.
...
| Surface | Corpus | Role |
|---------|--------|------|
| Public index | All public git | The free public authority asset |
| Org index (private) | A customer's repos | The internal enterprise provenance dashboard |

Build the aggregation engine once; point it at either corpus. This is the index analog of the
badge being "skinnable" per audience.
...
- **Two rows: who, then how much (decision, 2026-08-12).** The badge holds the two facts separately,
  ...
  - **Row 2 — the coverage — `known X%`.** How much of the origin's authorship *type* we actually
    ...
    narrative default *unknown*. We never promote unknown → human, nor assert
...
- **CoverageView** — a *computed, mutable* roll-up (provenance coverage %, tier distribution,
  ...
  tree, **an org's private repos**, or **the whole public corpus**
...
4. **The badge image is live-served and always linked; it is never treated as standalone
   proof.** (§9)
5. **The act of attribution is free; we order evidence and never assert truth.**
...
**Two paths into one record.**
- **The individual path** — reactive, artifact-level, authorship-centric: marshal evidence
  to outrank. Inherently nuanced; for the motivated creator and genuinely contested edge cases.
- **The enterprise path** — connect the source **once** (verify org identity, install
...
> (automation at the source), never grant a privileged override. This is what stops a lying powerful
> actor from crushing a truthful smaller creator.

**Reactive enterprises — speed, not outcome.** Most enterprises arrive *after*
...
identity verification, evidence-dossier assembly, guided sworn-ownership filing — plus a fast on-ramp
to external/legal resolution that writes back as a high-tier event. Speed never overrides the
ranking. A claimant who moves fast but lacks the facts still loses (a false sworn claim is perjury;
absent lineage stays absent).

**Residual risk, stated plainly.** The mirror of the small-creator-wins case is a powerful actor
filing a *false* sworn ownership claim against a small creator. We don't eliminate courtroom power
asymmetry,
...
**Consent, abuse & takedown are product, not "orthogonal policy".** For a product that publishes
inferences about non-consenting third parties and lets *anyone assert anything*, these must be
designed in, not punted. Required on the public path: (a) **consent-gated public name-attached
pages** + aggregate-only index + salted/HMAC private fingerprints + self-serve opt-out/takedown
(§8); (b) a **social/economic abuse model** the crypto red-team doesn't cover — **identity-cost on
asserted claims** (GitHub OAuth to assert, raising mass-squatting cost), **bounded "adjudication by
display"** (collapse low-tier/duplicate claims behind an "N other asserted claims" expander so a
contested page never renders unboundedly), and **corpus-poisoning detection** (one actor claiming
many unrelated repos → flag), wired to the trust-&-safety dashboard and sharing one mechanism with
API metering. Court-order takedowns remain operational, but consent/opt-out/abuse are **product**,
designed in.
...
### Why open extension isn't a giveaway
...
build it, and still concentrates the derived data at the hosted resolution/grant layer regardless of
...
- Producer-push / form-first as the spine (→ asker-pull + analyze-existing-history entry point).
