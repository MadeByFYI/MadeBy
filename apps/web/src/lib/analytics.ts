// Funnel instrumentation seam (review 2026-06-29, Concern A). The asserted→verified conversion is
// "the single most important number" (OPERATIONS §7) and is currently unmeasured. This builds the
// seam now so events fire from the first deploy; the PostHog transport activates when POSTHOG_KEY
// exists (blocked-on-atlas). Analytics must NEVER break a request — all failures are swallowed.

/** The conversion funnel, named once. asserted (analyze) → … → verified. */
export const FUNNEL = {
...
/** Fire a funnel event. No-op (dev log) until POSTHOG_KEY is set; never throws. */
