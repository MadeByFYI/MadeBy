...
export function proveCommand(args: string[]): void {
  ...
  if (res.error === "unrecognized-format") {
    console.error("madeby prove: unrecognized log format — no registered tool parser matched. (Claude Code is supported today.)");
    process.exit(1);
    return;
  }
  if (!res.written) {
    console.log(`madeby prove: ${res.message}. Nothing recorded.`);
    process.exit(0);
    return;
  }
  console.log(
    `madeby prove: wrote ${res.path} — [${res.tool}] ${res.matched} witnessed file(s) matched, ` +
