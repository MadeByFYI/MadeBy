...
import { captureLocalSpans, type CaptureLocalInput } from "./capture-local";
import { claudeCodeParser } from "./tool-parsers";
...
// The engine takes an explicit parser (the CLI detects + passes it); tests use the Claude Code one.
const run = (i: Omit<CaptureLocalInput, "parser">) => captureLocalSpans({ ...i, parser: claudeCodeParser });
...
describe("captureLocalSpans — witnessed, structurally-matched, honest", () => {
  ...
  it("still matches after reformatting (structural fingerprint survives whitespace/comment churn)", () => {
    ...
    const r = run({ transcript: write("src/math.ts", AI_SRC), repoRoot: ROOT, readFile: () => reformatted, commit: "abc", operatorId: "mac@madeby.fyi", now: NOW });
    expect(r.matched).toEqual(["src/math.ts"]);
  });

  it("does NOT attest AI work that isn't present in the committed file (discarded, honestly)", () => {
    const r = run({ transcript: write("src/math.ts", AI_SRC), repoRoot: ROOT, readFile: () => "a poem about the sea, nothing like the code\n", commit: "abc", operatorId: "mac@madeby.fyi", now: NOW });
  ...
  it("discards files the AI wrote that no longer exist in the repo", () => {
    const r = run({ transcript: write("src/gone.ts", AI_SRC), repoRoot: ROOT, readFile: () => null, commit: "abc", operatorId: "mac@madeby.fyi", now: NOW });
  ...
  it("ignores files outside the repo and our own .madeby records", () => {
    ...
    const r = run({ transcript, repoRoot: ROOT, readFile: () => AI_SRC, commit: "abc", operatorId: "m@x", now: NOW });
    expect(r.matched).toEqual(["src/keep.ts"]);
  });

  it("no parser (unrecognized tool) ⇒ honest no-op", () => {
    const r = captureLocalSpans({ transcript: write("src/x.ts", AI_SRC), repoRoot: ROOT, readFile: () => AI_SRC, commit: "abc", operatorId: "m@x", now: NOW });
